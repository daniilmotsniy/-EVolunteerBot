...
class API {
  ...
  /**
   * 
   * @param {Number} status The status of the coordinator orders
   * @returns 
   */
  getCooordinatorList(status = 2) {
    // console.warn(status);
  ...
  /**
   * Get a single user
   * @param id User id. Equals to "self" (get by the Access Token) if not specified
   * @returns Object
   */
  getUser(id = "self") {
  ...
  /**
   * 
   * @returns The PDF S3 link to download the report
   */
  getPDFReport() {
    return this.axiosInstance.get(`/api/order/pdf`);
  };
}
